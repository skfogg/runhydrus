---
title: "Parameterize and Run a HYDRUS Model"
author: "S. Katie Fogg"
date: "2026-04-16"
...
#### 1. Create a new HYDRUS model project in R using `create_hydrus_project()`

```{r new_proj_1, eval=TRUE}
fairfield_dual_perm_kbr <-  create_hydrus_project(
  project_name = "fairfield_dual_perm_kbr",
  parent_dir = "./examples",
  description = "Dual perm model, add artificial KBr solute",
  check_overwrite = FALSE
)
```

#### 2. Edit the parameterization of the model list using the functions with the same name as list components

```{r param_change_1, eval=TRUE}
## Model Units:
fairfield_dual_perm_kbr$model_units <- model_units(
  mass_unit = "mmol"
)
## Main Processes:
fairfield_dual_perm_kbr$main_processes <- main_processes(
  water_flow = T,
  root_water_uptake = T,
  root_growth = T,
  solute_transport = T
...
## Geometry:
fairfield_dual_perm_kbr$geometry <- geometry(
  number_materials = 3,
  profile_depth = 175,
...
## Root Growth:
fairfield_dual_perm_kbr$root_growth <- root_growth(
  root_growth_factor = 1,
  root_growth_depth = 2,
...
fairfield_dual_perm_kbr$time_variable_bc <- time_variable_bc(
  time_variable_bc = TRUE
)
## Water Flow Boundary Conditions:
fairfield_dual_perm_kbr$water_flow_bcs <- water_flow_bcs(
  upper_bc = "atm_bc_with_surface_runoff"
)
## Print Options:
fairfield_dual_perm_kbr$print_options <- print_options(
  times_to_print = data.frame(times = 1334:1698),
  print_times = FALSE,
  interval_output = 1
)
## Time Parameters:
fairfield_dual_perm_kbr$time_parameters <- time_parameters(
  final_model_time = 1698
)
## Root Water Uptake:
fairfield_dual_perm_kbr$root_water_uptake <- root_water_uptake(
  root_water_uptake_model = 1,
  root_water_uptake_params = data.frame(critical_stress_index = 0,
                                        h50 = -800,
                                        P3 = 3)
)
## Solute Transport:
fairfield_dual_perm_kbr$solute_transport <- solute_transport(
...
fairfield_dual_perm_kbr$soil_hydraulics <- soil_hydraulics(
  soil_hydraulic_model = 8,
...
#### 3. Write the parameterization in R to HYDRUS input files
...
## Run the HYDRUS model through R
...
## Reuse a HYDRUS model parameterization

```{r reuse_1, eval=FALSE, echo = F}
model_reuse <- read_hydrus_model("../examples/fairfield_dual_perm_kbr_working")
print(model_reuse)
```
